...
#     train_set_2 = Dataset(os.path.join(args.data_path, 'bn/new_train'), tokenizer=tokenizer, sequence_len=sequence_len,
#                       token_style=token_style, is_train=True, augment_rate=ar, augment_type=aug_type)
...
if args.language == 'bangla':
    train_set = Dataset(os.path.join(args.data_path, 'bn_inhouse/train.txt'), tokenizer=tokenizer, sequence_len=sequence_len,
                        token_style=token_style, is_train=True, augment_rate=ar, augment_type=aug_type)

    val_set = Dataset(os.path.join(args.data_path, 'bn_inhouse/val.txt'), tokenizer=tokenizer, sequence_len=sequence_len,
                      token_style=token_style, is_train=False)

    test_set_a = Dataset(os.path.join(args.data_path, 'bn_inhouse/test.txt'), tokenizer=tokenizer, sequence_len=sequence_len,
                           token_style=token_style, is_train=False)
    test_set = [val_set, test_set_a]
else:
    raise ValueError('Incorrect language argument for Dataset')
...
def train():
    with open(log_path, 'a') as f:
        f.write(str(args)+'\n')
    best_val_acc = 0
    for epoch in range(args.epoch): #start_epoch
        train_loss = 0.0
        train_iteration = 0
        correct = 0
        total = 0
